package oms.mmc.fastlist.util.statusbar;/*
...
public class QMUIStatusBarHelper {
    ...
    public static void whiteStatusBar(Activity activity) {
        statusBarColor(activity.getWindow(), 0xffffffff);
    }
...
//            }
    ...
    @TargetApi(19)
    public static void statusBarColor(Window window, @ColorInt int colorOn5x) {
        if (!supportTranslucent()) {
            // 版本小于4.4，绝对不考虑沉浸式
            return;
        }
...
//            }
    ...
    private static void initStatusBarHeight(Context context) {
        ...
        if (QMUIDeviceHelper.isTablet(context)
                && sStatusbarHeight > ScreenUtils.dp2px(context, STATUS_BAR_DEFAULT_HEIGHT_DP)) {
